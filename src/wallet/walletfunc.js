class Wallet {
    generation = () => {
        ...
        const sha2 = createHash("rmd160")
            .update(sha1)
            .digest();
        var checksum = ethereumjs.Buffer.Buffer.allocUnsafe(21);
        ...
        const pubHash = createHash("sha256")
            .update(pubKey)
            .digest();
        pubHashrmd = createHash("rmd160")
            .update(pubHash)
            .digest();

        var step4 = ethereumjs.Buffer.Buffer.allocUnsafe(21);
        step4.writeUInt8(0x00, 0);
        pubHashrmd.copy(step4, 1);
        Addr = bs58check.encode(step4);
        ...
        if (adr.substr(0, 12) == "bitcoincash:") {
            const Address = adr.substr(12, adr.length);
        } else {
            Address = adr;
        }
    }
